...
tulkinta. Koska huomiopainoille pätee \(A_{ij} \ge 0\) ja
...
o_i
\in
\operatorname{conv}(\{v_1,\dots,v_i\})
=
\left\{
  \sum_{j=1}^{i} \alpha_j v_j
  \;\middle|\;
  \alpha_j \ge 0,\;
...
\(x_{1:T}=(x_1,\dots,x_T)\), missä \(1 \le T \le L\), ja olkoon
\(E \in \mathbb{R}^{|\mathcal{V}| \times d}\) opittava
upotusmatriisi. Kun
\(e(x_i) \in \mathbb{R}^{|\mathcal{V}|}\) on tokenin \(x_i\)
one-hot-esitys, alkuperäinen esitysmatriisi määritellään riveittäin
muodossa
\begin{align*}
  H^{[0]}_{i,:}
  =
...
muuntamista jatkuvaksi esitysvektoriksi. Käytännössä kertolaskun
sijaan poimitaan tokenia vastaava upotusmatriisin rivi.
...
\(x_{1:i}\). Koko syötekontekstia seuraavan tokenin jakauma
...
\(x_{1:T}\) todennäköisyys voidaan esittää muodossa
$$
p_{\theta}(x_{1:T})
=
\prod_{t=1}^{T}
...
$$
missä \(x_{<t}=(x_1,\dots,x_{t-1})\).
Ensimmäisen tokenin ehdollinen jakauma tulkitaan edellä
sovitun aloitustokenikäytännön mukaisesti. Esitys koskee
tässä mallin konteksti-ikkunaan mahtuvaa sekvenssiä.
Kontekstirajan ylittäville sekvensseille mallin käyttämä
historia on rajattava erikseen.

Olkoon koulutusaineisto
\(\mathcal{D}=\{x^{(1)},\dots,x^{(N)}\}\), missä sekvenssin
\(x^{(r)}\) ennustettavien tokenien lukumäärä on \(T_r\).
...
havaittua oikeaa prefiksiä \(x_{<t}\) eikä mallin itsensä aiemmin
...
ja tavoitevastauksesta \(y=(y_1,\dots,y_K)\).
...
$$
\mathcal{L}_{\mathrm{instr}}(\theta;c,y)
=
-\sum_{t=1}^{K}
