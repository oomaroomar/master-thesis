...
tulkinta. Merkitään arvomatriisin \(V\) rivin \(j\) transpoosia
symbolilla \(\mathbf{v}_j=V_{j,:}^{\top}\).
Koska huomiopainoille pätee \(A_{ij} \ge 0\) ja
\(\sum_{j=1}^{T} A_{ij} = 1\), position \(i\) tulosvektori
$$
o_i = \sum_{j=1}^{T} A_{ij} \mathbf{v}_j
$$
...
Olkoon päiden lukumäärä \(h\). Kukin pää
\(k \in \{1,\dots,h\}\) suorittaa kausaalisen itsehuomion
...
\(W^O \in \mathbb{R}^{h d_v \times d}\) on opittava
parametrimatriisi.
...
usein \(d_k = d_v = d/h\), jolloin monipäisen itsehuomion
...
\left(
  ...
  R(i\theta_1),
  \dots,
  R\left(i\theta_{d_k/2-1}\right)
\right)
\in
\mathbb{R}^{d_k \times d_k}.
$$

Olkoon \(q_i \in \mathbb{R}^{d_k}\) position \(i\) kyselyvektori ja
\(k_j \in \mathbb{R}^{d_k}\) position \(j\) avainvektori. RoPE korvaa
nämä kierretyillä vektoreilla

$$
\widetilde q_i = R_i q_i,
\qquad
\widetilde k_j = R_j k_j.
$$
...
\(R(\alpha)^{\top}R(\beta)=R(\beta-\alpha)\). Sama ominaisuus pätee
...
positioiden suhteellisesta siirtymästä \(j-i\). RoPE liittää näin
...
sarakevektorille \(x \in \mathbb{R}^{d}\) muodossa
$$
\operatorname{FFN}(x)
=
W_2 \phi(W_1 x + b_1) + b_2,
$$
missä \(W_1 \in \mathbb{R}^{d_{ff} \times d}\),
\(W_2 \in \mathbb{R}^{d \times d_{ff}}\),
\(b_1 \in \mathbb{R}^{d_{ff}}\),
\(b_2 \in \mathbb{R}^{d}\) ja \(\phi\) on
...
=
\phi\left(
  H W_1^{\top} + \mathbf{1} b_1^{\top}
\right) W_2^{\top}
+
\mathbf{1} b_2^{\top},
$$
missä \(\mathbf{1} \in \mathbb{R}^{T}\) on ykkösvektori,
...
\mathbb{R}^{T \times d}.
$$
Olkoon \(z_i=Z_{i,:}^{\top} \in \mathbb{R}^{d}\) position \(i\)
lopullinen esitys sarakevektorina. Se muunnetaan sanaston
...
$$
missä
\(W_{\mathrm{out}} \in \mathbb{R}^{|\mathcal{V}| \times d}\)
ja \(b_{\mathrm{out}} \in \mathbb{R}^{|\mathcal{V}|}\)
ovat opittavia parametreja. Joissakin toteutuksissa siirtovektori
jätetään pois. Vektorin \(\ell_i\) komponentteja kutsutaan
...
\(v_{1:T}\) todennäköisyys voidaan esittää muodossa
$$
p_{\theta}(v_{1:T})
=
\prod_{t=1}^{T}
p_{\theta}(v_t \mid v_{<t}),
$$
missä \(v_{<t}=(v_1,\dots,v_{t-1})\).

Olkoon koulutusaineisto
\(\mathcal{D}=\{v^{(1)},\dots,v^{(N)}\}\), missä sekvenssin
\(v^{(r)}\) ennustettavien tokenien lukumäärä on \(T_r\).
...
havaittua oikeaa prefiksiä \(v_{<t}\) eikä mallin itsensä aiemmin
...
ja tavoitevastauksesta \(y=(v_1,\dots,v_K)\).
...
Kun SQL-kyselyn tokenijono on \(y=(v_1,\dots,v_K)\),
sen ehdollinen todennäköisyys voidaan kirjoittaa muodossa
$$
p_{\theta}(y \mid c)
=
\prod_{t=1}^{K}
p_{\theta}(v_t \mid c,v_{<t}).
$$
