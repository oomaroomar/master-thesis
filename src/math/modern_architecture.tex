...
\subsection{Dekooderilohko ja Pre-LN-rakenne}

Pelkkä itsehuomiomekanismi ei vielä riitä muodostamaan syvää arkkitehtuuria, sillä se on pohjimmiltaan pelkkä lineaaristen projektioiden ja konveksikombinaatioiden yhdistelmä. Jotta malli voi oppia monimutkaisia, epälineaarisia funktioita ja jotta se voidaan kouluttaa vakaasti useiden kerrosten läpi, huomiomekanismi yhdistetään muihin komponentteihin yhdeksi toistuvaksi yksiköksi, \emph{dekooderilohkoksi} (engl.\ \emph{decoder block}).

Dekooderilohko koostuu kahdesta pääkomponentista: edellä määritellystä kausaalisesta monipäisestä itsehuomiosta sekä positiokohtaisesta myötäkytkentäverkosta. Nämä kytketään toisiinsa normalisoinnin ja jäännöskytkentöjen (engl.\ \emph{residual connections}) avulla.

\subsubsection*{Normalisointi}

Syvien verkkojen sisäisten aktivaatioiden jakauman ajautumista ja gradienttien heilahtelua hallitaan normalisoinnilla. Nykyaikaisissa kielimalleissa standardiratkaisu on kerrosnormalisointi (engl.\ \emph{Layer Normalization}, LayerNorm) \cite{ba2016layer} tai sen laskennallisesti kevennetty muunnelma RMSNorm (engl.\ \emph{Root Mean Square Normalization}) \cite{zhang2019root}.

Normalisointi sovelletaan matriisin \(H \in \mathbb{R}^{T \times d}\) kuhunkin rivivektoriin \(h_i \in \mathbb{R}^{d}\) itsenäisesti. Esimerkiksi RMSNorm skaalaa esitysvektorin sen komponenttien neliöllisen keskiarvon avulla:
$$
\operatorname{RMS}(h_i) = \sqrt{\frac{1}{d} \sum_{k=1}^{d} (h_{i})_k^2 + \epsilon},
$$
missä \(\epsilon > 0\) on numeerista vakautta parantava pieni vakio. Tämän jälkeen vektori skaalataan komponenteittain opittavalla painovektorilla \(\gamma \in \mathbb{R}^d\):
$$
\operatorname{RMSNorm}(h_i) = \frac{h_i}{\operatorname{RMS}(h_i)} \odot \gamma.
$$
Matriisille \(H\) operaatio \(\operatorname{Norm}(H)\) tarkoittaa normalisoinnin soveltamista erikseen jokaiseen riviin.

\subsubsection*{Positiokohtainen myötäkytkentäverkko}

Itsehuomiomekanismin tehtävänä on jakaa informaatiota sekvenssin eri positioiden välillä. Tätä seuraa positiokohtainen myötäkytkentäverkko (engl.\ \emph{feed-forward network}, FFN), joka käsittelee kunkin position esitystä toisistaan riippumatta.

FFN vastaa luvussa \ref{sec:mlp} tarkasteltua kaksi- tai kolmekerroksista MLP-rakennetta, mutta sitä sovelletaan sellaisenaan jokaiseen rivivektoriin \(h_i^\top \in \mathbb{R}^{1 \times d}\):
$$
\operatorname{FFN}(h_i^\top) = \phi(h_i^\top W_1 + b_1) W_2 + b_2,
$$
missä \(W_1 \in \mathbb{R}^{d \times d_{\mathrm{ff}}}\), \(W_2 \in \mathbb{R}^{d_{\mathrm{ff}} \times d}\), \(b_1 \in \mathbb{R}^{1 \times d_{\mathrm{ff}}}\) ja \(b_2 \in \mathbb{R}^{1 \times d}\) ovat opittavia parametreja, ja \(\phi\) on epälineaarinen aktivaatiofunktio (kuten GELU tai Swish). Tyypillisesti sisäisen dimension suuruudeksi asetetaan \(d_{\mathrm{ff}} = 4d\).

Matriisimuodossa koko syötteelle \(H \in \mathbb{R}^{T \times d}\) kuvaus voidaan kirjoittaa tiiviisti
$$
\operatorname{FFN}(H) = \phi(H W_1 + \mathbf{1} b_1) W_2 + \mathbf{1} b_2,
$$
missä \(\mathbf{1} \in \mathbb{R}^{T \times 1}\) on ykkösvektori, joka toistaa bias-vektorin jokaiselle riville.

\subsubsection*{Lohkon kokonaisrakenne (Pre-LN)}

Nykyaikaisissa arkkitehtuureissa käytetään valtaosin Pre-LN-asettelua, jossa normalisointi suoritetaan ennen ali-lohkojen operaatioita suoraan jäännösväylästä otetulle syötteelle \cite{xiong2020layer}. Tällöin koko lohko voidaan kuvata funktiona \(\operatorname{DecoderBlock} \colon \mathbb{R}^{T \times d} \to \mathbb{R}^{T \times d}\), jonka laskenta etenee kahdessa vaiheessa:
$$
H^{(1)} = H + \operatorname{CausalMultiHead}(\operatorname{Norm}(H)),
$$
$$
\operatorname{DecoderBlock}(H) = H^{(1)} + \operatorname{FFN}(\operatorname{Norm}(H^{(1)})).
$$

Pre-LN-rakenteen keskeinen matemaattinen etu on suora identiteettiväylä syötteestä tulosteeseen. Koska operaatiot lisätään suoraan aiempaan tilaan (\(H + \dots\)), gradientit pääsevät virtaamaan ketjusäännön nojalla häviämättä syvienkin kerrosmäärien läpi, mikä tekee mallin optimoinnista merkittävästi stabiilimpaa verrattuna alkuperäiseen Post-LN-malliin.
