...
Itsehuomio on permutaatioekvivariantti eli ilman erillistä positiotietoa se ei kykene erottelemaan tokenien järjestystä. Olkoon
...
\(Q' = PQ\), \(K' = PK\) ja \(V' = PV\). Tällöin huomiopistematriisille
pätee

$$
\frac{Q'(K')^{\top}}{\sqrt{d_k}}
=
P\frac{QK^{\top}}{\sqrt{d_k}}P^{\top}.
$$
...
Syötteen tokenien permutointi siis ainoastaan permutoi tulosteet samalla
tavalla. Itsehuomiomekanismi ei sellaisenaan sisällä tietoa siitä, missä
järjestyksessä tokenit esiintyvät. Autoregressiivisessa mallissa
kausaalinen maski rikkoo tämän ominaisuuden mielivaltaisten
permutaatioiden suhteen, sillä se määrää, mitkä positiot ovat kunkin
tokenin menneisyydessä. Maski ei kuitenkaan sisällytä kyselyn ja avaimen
väliseen yhteensopivuuteen eksplisiittistä tietoa siitä, sijaitseeko
avain esimerkiksi yhden vai usean tokenin päässä kyselystä. Tämän vuoksi
Transformer tarvitsee erillisen tavan esittää tokenien sijainnit.
...
parillinen. RoPE jakaa vektorien koordinaatit
\(d_k/2\) kaksiulotteiseen pariin ja kiertää jokaista paria position
mukaisella kulmalla. Koordinaattiparille
\(\ell \in \{0,\dots,d_k/2-1\}\) määritellään kulmataajuus
