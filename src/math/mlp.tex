...
Luvussa \ref{sec:discrete} \(n\)-gram-malli esitettiin lukumääriin ja niiden
...
\[
  \theta_c = (\theta_{c,1}, \theta_{c,2}, \ldots, \theta_{c,|\mathcal{V}|}),
  \qquad
  \sum_{j=1}^{|\mathcal{V}|} \theta_{c,j} = 1,
  \qquad
  \theta_{c,j} \ge 0,
\]
missä \(\theta_{c,j} = P_\theta(w_t = v_j \mid c)\). Markov-oletuksen nojalla havaintoaineiston
\(\mathcal{W} = (w_1, \ldots, w_{|\mathcal{W}|})\) uskottavuus voidaan kirjoittaa muodossa
\[
  P(\mathcal{W};\theta)
  \approx
  \prod_{t=1}^{|\mathcal{W}|}
...
\[
  J(\theta;\mathcal{W})
  =
  -\log P(\mathcal{W};\theta)
  =
  -\sum_c \sum_{j=1}^{|\mathcal{V}|}
...
\[
  \min_{\theta_c}
  -\sum_{j=1}^{|\mathcal{V}|} C_{c,j} \log \theta_{c,j}
  \qquad
  \text{s.e.}
  \qquad
  \sum_{j=1}^{|\mathcal{V}|}\theta_{c,j} = 1.
...
\[
  ...
  C_{c,j}\log \theta_{c,j}
  +
  \lambda\left(1-\sum_{j=1}^{|\mathcal{V}|}\theta_{c,j}\right).
\]
...
saadaan
\[
  \lambda = \sum_{k=1}^{|\mathcal{V}|} C_{c,k},
\]
ja siten
\[
  \theta_{c,j}
...
luvussa \ref{sec:discrete} määritelty rivikohtaisesti normalisoitu
...
Tämä näkökulma tekee samalla näkyväksi mallin keskeisen rajoitteen, sillä jokaiselle
mahdolliselle kontekstille tarvitaan oma erillinen todennäköisyysjakaumansa.
Kontekstin pituuden kasvaessa parametrien määrä kasvaa nopeasti, jolloin malli
muuttuu harvaksi ja yleistää heikosti havaitsemattomiin konteksteihin.
Kielellisen variaation ja lauserakenteiden monimuotoisuuden käsittely on tällaiselle
taulukoidulle mallille haastavaa, sillä toisiaan merkitykseltään lähellä olevat
ilmaisut eivät jaa keskenään informaatiota. Tästä syystä on perusteltua siirtyä
malleihin, joissa ehdolliset todennäköisyydet opitaan jaetun parametrisaation avulla
jatkuvassa avaruudessa.
...
ulostulokerroksesta. Sen historialliset juuret ovat perseptronitutkimuksessa \cite{rosenblatt1958perceptron,rosenblatt1962principles}. Näissä töissä esitettiin varhaisia
...
Neuroverkkojen varhaista kehitystä hidasti merkittävästi kirja, jossa analysoitiin yksikerroksisten
...
lähtökohta syntyi työssä, jossa esitettiin käänteisen suunnan automaattinen derivointi menetelmä\cite{linnainmaa1970taylor}.
Tätä ajatusta sovellettiin myöhemmin neuroverkkoihin, jossa termi vastavirta-algoritmi (engl. \emph{backpropagation}) vakiintui \cite{werbos1974beyond, rumelhart1986learning}.

MLP voidaan ymmärtää funktion approksimaattorina, joka muodostaa
...
Olkoon \(c = (w_{t-n+1}, \dots, w_{t-1})\) tarkasteltava konteksti. Monikerroksisen
perseptronin syötevektori \(x(c) \in \mathbb{R}^{1 \times d_0}\), missä
\(d_0 = (n-1)d\), muodostetaan ketjuttamalla kontekstin tokenien upotusvektorit:
$$
x(c) = [e(w_{t-n+1}) \,\|\, e(w_{t-n+2}) \,\|\, \dots \,\|\, e(w_{t-1})],
$$
missä \(\|\) merkitsee vektorien vaakasuuntaista ketjuttamista. Monikerroksinen
perseptroni muuntaa tämän syötteen vaiheittain piirre-esitykseksi soveltamalla
peräkkäisiä affiineja muunnoksia ja epälineaarisia aktivaatioita. Jos mallissa on
\(L\) piilokerrosta, voidaan sen laskenta esittää muodossa
\begin{align*}
  h^{(0)} &= x(c),\\
  a^{(i)} &= h^{(i-1)} W^{(i)} + b^{(i)},
  & i = 1,\dots,L, \\
  h^{(i)} &= \phi^{(i)}\!\left(a^{(i)}\right),
  & i = 1,\dots,L,
\end{align*}
missä \(W^{(i)}\in\mathbb{R}^{d_{i-1}\times d_i}\) on kerroksen
\(i\) painomatriisi, \(b^{(i)}\in\mathbb{R}^{1\times d_i}\) sen bias-rivivektori,
\(a^{(i)}\in\mathbb{R}^{1\times d_i}\) affiinisen muunnoksen tulos ja
\(h^{(i)}\in\mathbb{R}^{1\times d_i}\) kerroksen \(i\) piiloesitys rivivektorina.
Funktio \(\phi^{(i)}\) on kerroksen epälineaarinen aktivaatiofunktio.

Viimeisestä piilokerroksesta muodostetaan ulostulon
normalisoimattomat pistemäärät eli logitit rivivektorina
$$
\ell = h^{(L)} W^{(\mathrm{out})} + b^{(\mathrm{out})},
$$
missä \(W^{(\mathrm{out})}\in\mathbb{R}^{d_L\times |\mathcal{V}|}\),
\(b^{(\mathrm{out})}\in\mathbb{R}^{1\times |\mathcal{V}|}\) ja \(\ell\in\mathbb{R}^{1\times |\mathcal{V}|}\).
Vektorin \(\ell\) komponentti \(\ell_j\) vastaa sanaston tokenin \(v_j\)
...
=
\frac{\exp(\ell_j)}{\sum_{k=1}^{|\mathcal{V}|}\exp(\ell_k)},
\qquad j=1,\dots,|\mathcal{V}|.
$$
...
upotusvektoreiksi \(v \in \mathbb{R}^{1\times d}\), malli oppii esittämään sanojen
...
sijaitsevat lähellä niitä yhteisessä avaruudessa \cite{bengio2003neural}.Tämä ominaisuus on keskeinen käyttäjän
tiedontarpeiden tulkinnassa, sillä se antaa mallille valmiuden yleistää
kielellisiin variaatioihin ja synonyymisiin ilmaisuihin ilman erillisiä
tasoitusmenetelmiä \cite{huang2013learning}.

Opittavat painomatriisit ja bias-vektorit jakautuvat kaikkien kontekstien kesken,
joten jokainen harjoitusnäyte päivittää mallin kokonaisesitystä. Parametrien
jakaminen tekee neuroverkkopohjaisesta lähestymistavasta joustavamman ja
muistikapasiteetiltaan tehokkaamman kuin diskreetti taulukkoesitys
\cite{bengio2003neural,goodfellow2016deep,jurafsky2023speech}. Samalla se
luo arkkitehtuurisen perustan laajemmille malleille, joissa syötteen
vaihtelevuudesta huolimatta pyritään säilyttämään tuotetun tulosteen semanttinen
eheys.
