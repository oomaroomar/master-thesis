...
(engl.\ \emph{feed-forward network}, FFN). Toisin kuin luvussa \ref{sec:mlp}
...
Koulutuksessa kunkin ennusteen syötteenä käytetään aineistossa havaittua
oikeaa prefiksiä mallin itsensä tuottamien tokenien sijasta.
Kun tavoitteena on mallintaa sekvenssi \((w_1, \dots, w_T)\), mallille syötetään
aloitustokenilla \(\langle s \rangle\) aloitettu jono
$$
x = (\langle s \rangle, w_1, w_2, \dots, w_{T-1}),
$$
ja vastaavaksi tavoitejonoksi asetetaan
$$
y = (w_1, w_2, \dots, w_T).
$$

Kausaalisen maskin ansiosta syötteen positiossa \(t\) laskettu piiloesitys
riippuu vain alijonosta \((\langle s \rangle, w_1, \dots, w_{t-1})\).
Siten position \(t\) logitit \(\ell_t\) ennustavat suoraan kohdetokenia
\(y_t = w_t\). Kaikkien \(T\) position ennusteet ja häviöt voidaan näin
laskea yhdellä eteenpäinlaskennalla rinnakkain ilman, että tulevaisuuden
informaatio vuotaa menneisyyteen.

Esikoulutettua mallia voidaan edelleen mukauttaa
ohjeiden noudattamiseen valvotulla ohjevirityksellä.
Ohjevirityksessä koulutusesimerkki muodostuu syötekontekstista
\(c = (c_1, \dots, c_M)\) ja tavoitevastauksesta \(y = (y_1, \dots, y_K)\).
Koko mallille annettava syötejakso on näiden ketjutus
\((c_1, \dots, c_M, y_1, \dots, y_{K-1})\).

Ensimmäinen vastaustoken \(y_1\) ennustetaan kontekstin viimeisen tokenin
\(c_M\) kohdalta, ja kukin seuraava token \(y_t\) ennustetaan sitä edeltävän
vastaustokenin \(y_{t-1}\) kohdalta. Häviö lasketaan ainoastaan vastauksen
positioista asettamalla kontekstipositioiden häviömaski nollaksi
$$
...
$$
Syötekonteksti \(c\) vaikuttaa vastaustokenien piiloesityksiin itsehuomion
kautta, vaikka sen omille positioille ei lasketa ennustehäviötä.
