...
- Query 2, coutn 4, incorrect
  Note: with this prompt the model didn't apply the filter direction = 'inbound', isntead had direction_original = 'inbound'. We should assume the former when we are filtering by service level. The different queries only differ in that one of the counts using the distinct keyword.
...
- Query 2, count 1, correct, semantically same as q1
- Query 3, count 1, incorrect a bit, takes "distinct" at wrong point
- Query 4, count 1, wrong, lacks direction = inbound
